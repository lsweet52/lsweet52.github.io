...
-- ==================
-- 实验8.1
-- (1).1无参数的存储过程，定义一个存储过程，更新所有订单的（汉税折扣价）总价
delimiter $$
...
begin
    ...
    set o.total_price = l.total where total is not null;
end $$

delimiter ;

-- (1).2执行存储过程proc_caltotalprice()
call proc_caltotalprice();

-- (2).1有参数的存储过程，定义一个存储过程，更新给定订单的（含税折扣价）总价
delimiter $$

create procedure proc_caltotalprice4order(okey int)
begin
    update orders o set total_price = (
        select sum(extended_price * (1 - discount) * (1 + tax))
        from lineitem l where o.order_key = l.order_key and l.order_key = okey
    ) where o.order_key = okey;
end $$
...
create procedure proc_caltotalprice4customer(p_custname varchar(25))
begin
    declare l_custkey int;

    select cust_key into l_custkey from customer
        where name = trim(p_custname) limit 1;

    update orders o set total_price = (
        select sum(extended_price * (1 - discount) * (1 + tax))
        from lineitem l where o.order_key = l.order_key
        ) where o.cust_key = l_custkey;
end $$

delimiter ;

-- (3).2执行存储过程proc_caltotalprice4customer()
call proc_caltotalprice4customer('顾客A');

-- (3).3查看存储过程执行结果
select * from orders where cust_key = (
    select cust_key from customer where name = '顾客A'
    );

-- (4).1有输出参数的存储过程，定义一个存储过程，更新某个顾客的所有订单的（含税折扣价）总价
delimiter $$

create procedure proc_caltotalprice4customer2(p_custname varchar(25),
                                              out p_totalprice decimal(15,2))
begin
    declare l_custkey int;
    select cust_key into l_custkey from customer where name = trim(p_custname);

    update orders o set total_price = (
        select sum(extended_price * (1 - discount) * (1 + tax)) from lineitem l
        where o.order_key = l.order_key
        ) where o.cust_key = l_custkey;

    select sum(total_price) into p_totalprice
    from orders where cust_key = l_custkey;
end $$

delimiter ;

-- (4).2执行存储过程proc_caltotalprice4customer2()
call proc_caltotalprice4customer2('顾客A', null);

-- (4).3查看存储过程执行结果
select sum(total_price) from orders where cust_key = (
    select customer.cust_key from customer where name = '顾客A'
    );

-- (5)(6)修改存储过程， 删除存储过程
delimiter $$

create procedure caltotalprice4order(okey int)
begin
    update orders o set total_price = (
        select sum(extended_price * (1 - discount) * (1 + tax))
        from lineitem l where o.order_key = l.order_key and l.order_key = okey
...
drop procedure proc_caltotalprice4order;


-- ======================

-- 实验8.2
-- (1).1、定义一个自定义函数，更新所有订单的总价，并返回所有订单的总价之和
delimiter $$

create procedure fun_caltotalprice(out res decimal(15,2))
begin
    update orders o
    join (
        select order_key, sum(extended_price * (1 - discount) * (1 + tax)) as total
    ...
    where o.order_key in (select order_key from lineitem);

    select sum(total_price) into res from orders;
end$$

delimiter ;

-- （1）.2、执行自定义函数fun_caltotalprice()
call fun_caltotalprice(@total_price);
select @total_price;

-- (2).1、定义一个自定义函数，更新并返回给定订单的总价
delimiter $$

create procedure fun_caltotalprice4order(
    in p_okey int,
    out res decimal(15,2)
)
begin
    update orders o
    join (
        select order_key, sum(extended_price * (1 - discount) * (1 + tax)) as total
        from lineitem
        where order_key = p_okey
        group by order_key
    ) l on o.order_key = l.order_key
    set o.total_price = l.total
    where o.order_key = p_okey;

    select total_price into res from orders where order_key = p_okey;
end$$

delimiter ;

-- (2).2、执行fun_caltotalprice4order()
call fun_caltotalprice4order(1, @total);
select @total;

-- (3).1、定义一个自定义函数，计算并返回某个顾客的所有订单的总价
delimiter $$

create procedure fun_caltotalprice4customer(
    in p_custname varchar(25),
    out res decimal(15,2)
)
begin
    declare l_custkey int;

    select cust_key into l_custkey from customer where name = trim(p_custname) limit 1;
    ...
    select sum(total_price) into res from orders where cust_key = l_custkey;
end$$

delimiter ;


-- (3).2、执行fun_caltotalprice4customer()
call fun_caltotalprice4customer('顾客B', @total_price);
select @total_price;


-- (4).1、定义一个自定义函数，计算并返回某个顾客的所有订单的总价
delimiter $$

create procedure fun_caltotalprice4customer2(
    in p_custname varchar(25),
...
begin
    ...
    select cust_key into l_custkey
    from customer
    ...
    select sum(total_price) into p_totalprice
    from orders
    where cust_key = l_custkey;

end $$

delimiter ;

-- (4).2、执行fun_caltotalprice4customer2()
call fun_caltotalprice4customer2('顾客C', @totalprice);
select @totalprice;
...
create procedure caltotalprice4order_fun(
    in p_okey int,
    out res decimal(15,2)
)
begin
...
-- (6)删除自定义函数
drop procedure caltotalprice4order_fun;




-- 实验8.3游标实验
-- （1）普通游标，定义一个存储过程，用游标实现计算所有订单的总价
delimiter $$

create procedure proccursor_caltotalprice()
begin
    declare l_orderkey int;
    declare l_totalprice decimal(15,2);
    ...
    cursor_loop: loop
        ...
        into l_totalprice
        from lineitem
        where order_key = l_orderkey;

        -- 更新订单总价
        update orders
        set total_price = l_totalprice
        where order_key = l_orderkey;
...
-- (1).2执行proccursor_caltotalprice()
call proccursor_caltotalprice();

-- (2).1定义一个存储过程，用游标实现计算所有订单的总价
delimiter $$

create procedure procrefcursor_caltotalprice()
begin
    declare l_orderkey int;
    declare l_totalprice decimal(15,2);
    declare done int default 0;
...
-- (2).2、执行proccursor_caltotalprice()
call procrefcursor_caltotalprice();


-- (3).1、定义一个存储过程，用游标实现计算所有订单的总价
delimiter $$
...
-- (3).2、执行proccursor_caltotalprice()
 call procreccursor_caltotalprice();


-- (4)带参数的游标，定义一个存储过程，用游标实现计算指定国家的用户订单的总价
delimiter $$

create procedure procparacursor_caltotalprice(p_nationname varchar(25))
begin
    declare l_orderkey int;
    declare l_totalprice decimal(15,2);
    declare done int default 0;
    ...
    declare mycursor cursor for
        ...
        where trim(n.name) = trim(p_nationname);
...
-- (4).2 、执行procparacursor_caltotalprice()
call procparacursor_caltotalprice('中国');

-- 实验10.1
-- (1).使用explain命令查看查询执行计划
/*
 查看part，partsupp, supplier三个表连接查询的查询执行计划
 */
explain select * from part p, partsupp ps, supplier s
    where p.part_key = ps.part_key and ps.supp_key = s.supp_key
        and p.name = '零件B' order by s.address desc, s.name, p.part_key;

-- (2).利用索引优化查询性能
create index idx_part_name on part(name);

explain select * from part p, partsupp ps, supplier s
    where p.part_key = ps.part_key and ps.supp_key = s.supp_key and p.name = '零件B'
    order by s.address desc , s.name, p.part_key;

-- (3)优化SQL语句
-- 1.
# select * from orders where order_key in(
#     select order_key from lineitem where
#     )
